    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="presentation">
      <div className="w-full max-w-lg rounded-2xl bg-surface p-6 shadow-xl sm:p-8" role="dialog">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-heading" id="new-task-title">New task</h2>
            <p className="mt-2 text-sm text-body">Add a task to your workspace.</p>
          </div>
          <Button
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <Button disabled={isSubmitting} type="submit">{isSubmitting ? 'Creating...' : 'Create task'}</Button>
        </form>
      </div>
    </div>
  )
